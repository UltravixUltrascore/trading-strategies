...
def prepare_aggregated_dataset(df, tickers):
    ...
    for ticker in tickers:
        ...
        df_ticker = add_moving_average(df_ticker)  # Aggiunta della SMA50
...
def apply_ml_strategy_aggregated(df):
    ...
    features = ['Open', 'High', 'Low', 'Close', 'Volume', 'ATR', 'SMA50',
                'Doji', 'Engulfing_Bullish', 'Engulfing_Bearish', 'Hammer',
    ...
    for train_index, test_index in tscv.split(X):
        ...
        best_params = optimize_hyperparameters(X_train, y_train)
         # Modello XGBoost con iperparametri ottimizzati
        model = XGBClassifier(
...
def apply_strategy_with_aggregated_model(df, tickers):
    ...
    df_result, best_model = apply_ml_strategy_aggregated(df_aggregated)
    ...
    return portfolio_df, best_model
...
portfolio, best_model = apply_strategy_with_aggregated_model(df, tickers)
...
def feature_importance_analysis(model, features):
    ...
    feature_importance_df = pd.DataFrame({'Feature': features + ['Ticker_Code'], 'Importance': importance})
